...
public class ProtractorCountdown extends View {
    ...
    private int mCounterSize = 14;

    private int mTickOffset = 12;
    private int mTickLength = 10;
    private int mTickWidth = 2;//Width Tick between Number
    private int mTickProgressWidth = 2;
    private int mAngle = 0;
    private boolean mTouchInside = true;
    private boolean mEnabled = false;
    private TicksBetweenLabel mTicksBetweenLabel = TicksBetweenLabel.ONE;//For Show Number
    private int mTickIntervals = 15;//For Separate by value 15
    private double mTouchAngle = 0;
    private float mTouchIgnoreRadius;
    private Typeface myTypeFace=null;
    private String[] splitText = {"60"};
    private String text = "";


    private OnProtractorCountDownTimerListener mOnProtractorCountDownTimerListener=null;

    //Interface for event CountDown listener
    public interface OnProtractorCountDownTimerListener {
        void onEndProtractorCountDownTimer(boolean endedTimer );
    }
    ...
    public ProtractorCountdown(Context context) {
        super(context);
        init(context, null, 0);
    }
    public ProtractorCountdown(Context context,Typeface typeface) {
        super(context);
        myTypeFace=typeface;
        init(context, null, R.attr.ProtractorCountDownStyle);
    }

    public ProtractorCountdown(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.context=context;
        this.attrs=attrs;
        init(context, attrs, R.attr.ProtractorCountDownStyle);
    }

    public ProtractorCountdown(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        mCounterSize = (int) (mCounterSize * DENSITY);
        ...
        if (attrs != null) {
            final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.ProtractorCountDown, defStyle, 0);
            Drawable thumb = array.getDrawable(R.styleable.ProtractorCountDown_thumb);
            if (thumb != null) {
                mThumb = thumb;
            }
            ...
            //Dimensions
            mAngleTextSize = (int) array.getDimension(R.styleable.ProtractorCountDown_angleTextSize, mAngleTextSize);
            mArcProgressWidth = (int) array.getDimension(R.styleable.ProtractorCountDown_progressWidth, mArcProgressWidth);
            mTickOffset = (int) array.getDimension(R.styleable.ProtractorCountDown_tickOffset, mTickOffset);
            mTickLength = (int) array.getDimension(R.styleable.ProtractorCountDown_tickLength, mTickLength);
            mArcWidth = (int) array.getDimension(R.styleable.ProtractorCountDown_arcWidth, mArcWidth);
            //Integers
            mAngle = array.getInteger(R.styleable.ProtractorCountDown_angle, mAngle);
            mTickIntervals = array.getInt(R.styleable.ProtractorCountDown_tickIntervals, mTickIntervals);
            //Colors
            arcColor = array.getColor(R.styleable.ProtractorCountDown_arcColor, arcColor);
            arcProgressColor = array.getColor(R.styleable.ProtractorCountDown_arcProgressColor, arcProgressColor);
            textColor = array.getColor(R.styleable.ProtractorCountDown_textColor, textColor);
            textProgressColor = array.getColor(R.styleable.ProtractorCountDown_textProgressColor, textProgressColor);
            tickColor = array.getColor(R.styleable.ProtractorCountDown_tickColor, tickColor);
            tickProgressColor = array.getColor(R.styleable.ProtractorCountDown_tickProgressColor, tickProgressColor);
            //Boolean
            mRoundedEdges = array.getBoolean(R.styleable.ProtractorCountDown_roundEdges, mRoundedEdges);
            mEnabled = array.getBoolean(R.styleable.ProtractorCountDown_enabled, mEnabled);
            mTouchInside = array.getBoolean(R.styleable.ProtractorCountDown_touchInside, mTouchInside);
            int ordinal = array.getInt(R.styleable.ProtractorCountDown_ticksBetweenLabel, mTicksBetweenLabel.ordinal());
        ...
        textPaint.setTextSize(mCounterSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 360; i >= 180; i -= mTickIntervals) {
            ...
            if (count == mTicksBetweenLabel.ordinal()) {
                ...
                canvas.drawText("" + (timeSecond-((360 - i)*timeSecond/180)), (float) midTickX, (float) midTickY, (mAngle <= 359 - i) ? mTickTextPaint : mTickTextColoredPaint);
                count = 0;
            } else {
                //for tick
                canvas.scale(-1, 1, mArcRect.centerX(), mArcRect.centerY());
        ...
        for (String s : splitText) {
            ...
            canvas.drawText(s, this.getWidth() / 2 - horizontalTextOffset,
                    (this.getHeight()/6)*5  + verticalTextOffset, textPaint);
        }
    }
    ...
    public void onProgressRefresh(int angle, boolean fromUser) {
    ...
    private void updateAngle(int angle, boolean fromUser) {
        ...
        updateThumbPosition();
        setText(timeSecond- (int) (mAngle*(timeSecond/180.0))+"");
        invalidate();
    }
    ...
    public void setOnProtractorCountDownTimerListener(OnProtractorCountDownTimerListener l) {
        mOnProtractorCountDownTimerListener = l;
    }

    public OnProtractorCountDownTimerListener getmOnProtractorCountDownTimerListener() {
        return mOnProtractorCountDownTimerListener;
    }
    ...
    public void setMyTypeFace(Typeface tf) {
        this.myTypeFace = tf;
        mTickPaint.setTypeface(myTypeFace);
        mArcPaint.setTypeface(myTypeFace);
        mArcProgressPaint.setTypeface(myTypeFace);
        mTickProgressPaint.setTypeface(myTypeFace);
        mTickTextPaint.setTypeface(myTypeFace);
        mTickTextColoredPaint.setTypeface(myTypeFace);
        textPaint.setTypeface(myTypeFace);
        invalidate();
    ...
    public void CountTimeSet(int timeSecond)
    {
        this.timeSecond=timeSecond;
        invalidate();
        int milliDiff=timeSecond*1000;
        CountDownTimer countDownTimer=  new CountDownTimer(milliDiff, 1000) {

            @Override
            public void onTick(long millisUntilFinished) {
                onProgressRefresh((int)((timeSecond- (int) ((millisUntilFinished /1000)))*180.0/timeSecond),true);
            ...
            public void onFinish() {
                if (mOnProtractorCountDownTimerListener != null) {
                    mOnProtractorCountDownTimerListener.onEndProtractorCountDownTimer(true);
                }
